...
- Logging is per-set, one set revealed at a time (you can't do set 2 before set 1); warm-ups are their own section above working sets. Exercise cards are an accordion (only one open at a time, owned by the parent form) with up/down reordering. The exercise name in a log card is plain text with a dedicated info icon (ⓘ) next to it to open the exercise-detail popup — the name itself isn't a click target, so "view info" and "expand to log" don't compete on the same tap.
...
- `Goal` (weight or strength type, optional target_date, `is_active`) is jointly owned: a trainee can always add/edit/remove their own goals, and once a trainer is assigned that trainer can too (`GoalWritePermission`) — either side's changes are visible to both, with no read-only lockout in either direction.
...
- Destructive actions (delete a goal/session/exercise/...) confirm via the app-styled `components/ConfirmDialog.tsx`, never the browser's native `window.confirm`.
