...
- Core models: `DietPlan` (trainee FK) → `ReferenceMeal` (diet_plan FK, label/order) → `ReferenceMealItem` (meal FK, food_item FK, reference_weight_grams) → `FoodItem` (name, barcode nullable, per-100g nutrition, source: seeded/off) → `FoodLog` (trainee FK, reference_meal_item FK, actual_weight_grams, logged_at).
...
6. ⏳ **Next up.** Scaffold React (Vite) frontend + auth (login, role-aware routing for trainer vs. trainee). Open decisions to confirm with the user before/while starting: JS vs. TypeScript for the frontend (not yet decided); how the dev frontend talks to the Django API — Vite dev-server proxy to `localhost:8000` (avoids CORS entirely, simplest for Stage 1) vs. `django-cors-headers` (not yet installed); where the auth token gets stored client-side (e.g. localStorage) and how the axios/fetch client attaches it.
7. Trainer UI: build workout plan, build diet plan (reference meals).
8. Trainee UI: `WorkoutLogger` (pick day → log sets), `FoodLogEntry` (pick meal slot → enter weight → see computed macros).
9. Trainee/trainer views: session history, adherence view (planned vs. actual).
10. End-to-end smoke test with a trainer + a couple of trainees.
...
- Food log linkage: `FoodLog` entries link to a specific `ReferenceMealItem`, not a flat/unstructured log, so planned-vs-actual adherence is directly comparable.
